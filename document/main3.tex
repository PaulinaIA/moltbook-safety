...
El proyecto implementa un pipeline completo de datos que abarca desde la recolección mediante \textit{web scraping} asíncrono con múltiples workers en paralelo, el almacenamiento en una base de datos relacional PostgreSQL en AWS RDS, el procesamiento en capas siguiendo la arquitectura Medallion, hasta el entrenamiento de un modelo de regresión con H2O AutoML para predecir el karma de los usuarios a partir de sus características de actividad e interacción en la plataforma.
...
\begin{enumerate}
    \item Dise\~nar un scraper robusto y escalable con Playwright, tanto s\'incrono como as\'incrono con m\'ultiples workers en paralelo, para extraer datos de moltbook.com con rate limiting, reintentos y cach\'e HTML.
    \item Almacenar los datos crudos en una base de datos PostgreSQL en AWS RDS con un esquema relacional normalizado con foreign keys e \'indices.
    \item Implementar una arquitectura Medallion (Silver/Gold) con Polars utilizando evaluaci\'on lazy para eficiencia en memoria, generando m\'ultiples datasets denormalizados a nivel de usuario, post y comentario.
...
Para este proyecto de ingeniería de datos hemos diseñado un pipeline modular que inicia con la extracción de los datos de \texttt{moltbook.com} utilizando técnicas de \textit{web scraping} asíncrono con Playwright y múltiples workers en paralelo. Estos datos extraídos se almacenan en una base de datos relacional PostgreSQL en AWS RDS que luego pasan por el módulo Silver, que se encarga de transformar los datos crudos del esquema relacional en datasets columnares optimizados mediante Polars con evaluación \textit{lazy}. Posteriormente, la capa Gold construye múltiples datasets de modelado mediante agregaciones complejas a nivel de usuario, post y comentario.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[stage, fill=accentblue!80, right=of scraping] (sqlite) {PostgreSQL};
...
En \texttt{src/scraper/} implementamos la extracción de datos con Playwright incluyendo la infraestructura de navegador tanto síncrona como asíncrona con múltiples workers en paralelo, descubrimiento de URLs, parsers HTML y los orquestadores de scraping. La capa de persistencia en \texttt{src/database/} define los modelos de datos, gestiona conexiones PostgreSQL y proporciona operaciones CRUD genéricas con upsert.
El procesamiento de datos en \texttt{src/processing/} aplica la arquitectura Medallion realizando la limpieza de los datos con Polars Lazy en la capa Silver e ingeniería de features en la capa Gold generando tres datasets denormalizados, la cual la complementamos haciendo el análisis exploratorio utilizando PySpark.
...
\hspace{0.3cm}base.py & BaseScraper s\'incrono con Playwright, rate limiter, cache \\
\hspace{0.3cm}async\_base.py & AsyncBaseScraper con m\'ultiples workers en paralelo \\
...
\hspace{0.3cm}scrapers.py & MoltbookScraper: orquestador s\'incrono \\
\hspace{0.3cm}async\_scrapers.py & AsyncMoltbookScraper: orquestador as\'incrono masivo \\
\midrule
\textbf{src/database/} & \textbf{Capa de persistencia} \\
\hspace{0.3cm}models.py & Dataclasses (User, Post, Comment, SubMolt) \\
\hspace{0.3cm}connection.py & Context manager para conexiones PostgreSQL y SQLite \\
...
\hspace{0.3cm}\_\_main\_\_.py & Comandos Click: scrape, scrape-massive, build, train, status, enrich-users, init-db \\
\midrule
...
\hspace{0.3cm}(PostgreSQL en AWS RDS) & Base de datos remota \\
...
\subsection{Scraping As\'incrono Masivo}

Para escalar la recolecci\'on de datos a cientos de miles de registros, implementamos un segundo orquestador \texttt{AsyncMoltbookScraper} basado en la API as\'incrona de Playwright. Este scraper utiliza m\'ultiples p\'aginas de navegador en paralelo controladas por un \texttt{asyncio.Semaphore} para limitar la concurrencia, y un \texttt{asyncio.gather} para ejecutar las solicitudes de forma paralela.

\begin{lstlisting}[language=Python, caption={Scraper as\'incrono con workers paralelos (simplificado)}]
class AsyncMoltbookScraper:
    async def scrape_all_massive(self, max_submolts=500, ...):
        # Phase 1: Discover users + submolts in parallel
        user_urls, submolt_urls = await asyncio.gather(
            self.discover_all_users(...),
            self.discover_all_submolts(...),
        )
        # Phase 2: Scrape user profiles
        users = await self.scrape_users_parallel(user_urls)
        # Phase 3: Scrape submolts + posts + comments
        #          with cross-discovery loop (max 5 rounds)
        while current_urls and round_num < 5:
            results = await self.scrape_submolts_parallel(...)
            discovered = results["discovered_submolt_names"]
            new_names = discovered - scraped_names
            current_urls = [f"/m/{n}" for n in new_names]
        # Phase 4: Enrich incomplete user profiles
        incomplete = self.db_ops.get_incomplete_user_names()
        await self.scrape_users_parallel(urls, force_refresh=True)
\end{lstlisting}

El flujo de 4 fases permite maximizar la recolecci\'on de datos. La Fase 1 descubre URLs en paralelo, la Fase 2 scrapea los perfiles de usuario, la Fase 3 implementa un ciclo de \textbf{cross-discovery} donde al scrapear los posts de cada submolt se descubren referencias a otros submolts no conocidos que se agregan a la cola de scraping, y la Fase 4 enriquece los perfiles de usuario que quedaron incompletos (karma = 0 o sin descripci\'on) con un cooldown previo para evitar el rate limiting del servidor.

El bloqueo de recursos innecesarios (im\'agenes, CSS, fuentes) en cada p\'agina del navegador reduce significativamente el consumo de ancho de banda y acelera la carga de las p\'aginas, ya que solo necesitamos el HTML para el parsing.
...
El esquema de la base de datos PostgreSQL en AWS RDS consta de 5 tablas con relaciones de foreign key e \'indices de rendimiento. Elegimos PostgreSQL por su capacidad de manejar escrituras concurrentes desde m\'ultiples workers del scraper as\'incrono y por su despliegue en la nube con AWS RDS.
...
\begin{lstlisting}[language=SQL, caption={Esquema DDL de la base de datos PostgreSQL}]
...
CREATE TABLE IF NOT EXISTS posts (
    ...
    id_user TEXT NOT NULL REFERENCES users(id_user),
    id_submolt TEXT REFERENCES sub_molt(id_submolt),
    title TEXT,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS comments (
    ...
    id_user TEXT NOT NULL REFERENCES users(id_user),
    id_post TEXT NOT NULL REFERENCES posts(id_post),
    description TEXT,
    date TEXT,
    rating INTEGER DEFAULT 0,
    scraped_at TEXT NOT NULL
...
CREATE TABLE IF NOT EXISTS user_submolt (
    id_user TEXT NOT NULL REFERENCES users(id_user),
    id_submolt TEXT NOT NULL REFERENCES sub_molt(id_submolt),
...
Para la gesti\'on de conexiones implementamos un context manager \texttt{get\_connection} que garantiza commit autom\'atico en caso de \'exito y rollback en caso de error, cerrando siempre la conexi\'on al finalizar. El sistema soporta tanto SQLite (para desarrollo local) como PostgreSQL (para producci\'on). La funci\'on \texttt{get\_postgres\_connection} utiliza \texttt{psycopg2} para conectarse a la instancia de AWS RDS, donde PostgreSQL maneja nativamente la integridad referencial de las foreign keys y la concurrencia de escrituras m\'ultiples desde los workers del scraper as\'incrono, utilizando \texttt{execute\_batch} de \texttt{psycopg2.extras} para operaciones de upsert masivas.
...
Los datos almacenados en PostgreSQL act\'uan como la capa Bronze de nuestra arquitectura Medallion, conteniendo los datos tal como se obtuvieron del scraping. En la siguiente secci\'on veremos c\'omo estos datos crudos se transforman progresivamente hasta llegar a m\'ultiples datasets listos para el modelado.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[layer, draw=accentblue, fill=accentblue!10] (bronze) {
            \textbf{Bronze}\\[2pt]
            {\scriptsize PostgreSQL -- Datos crudos}\\
            {\scriptsize 5 tablas, FK, AWS RDS}
        };
        ...
        \node[layer, draw=accentorange, fill=accentorange!10, right=1.5cm of silver] (gold) {
            ...
            {\scriptsize 3 datasets, hasta 27 cols}
        };
...
\begin{itemize}
    \item \textbf{Bronze (PostgreSQL):} Datos crudos tal como se obtienen del scraping.
...
La capa Silver carga los datos crudos desde PostgreSQL, los limpia y los almacena en formato Parquet columnar. La funci\'on \texttt{load\_table\_to\_lazy} lee cada tabla de PostgreSQL usando \texttt{pl.read\_database\_uri} y la convierte a \texttt{LazyFrame} con el m\'etodo \texttt{.lazy()}, lo que nos permite encadenar todas las operaciones de limpieza sin ejecutarlas hasta el final.
...
La variable \texttt{comment\_stats} sigue el mismo patr\'on de agregaci\'on que \texttt{post\_stats}, agrupando los comentarios por \texttt{id\_user} para calcular el conteo, rating total, rating promedio y longitud promedio de los comentarios. Despu\'es de unir ambas agregaciones con los datos de usuarios mediante \texttt{left join}, rellenamos los valores nulos con cero (para usuarios sin posts o sin comentarios) y creamos features derivados como el \texttt{follower\_ratio}, la \texttt{total\_activity} y flags binarios.

\subsubsection{Dataset a nivel de Post}

Adem\'as del dataset a nivel de usuario, construimos \texttt{engineer\_post\_features} que genera un dataset denormalizado con 24 columnas por post. Cada fila une el post con las features de su autor (karma, followers, follower ratio, etc.), la comunidad donde fue publicado, y m\'etricas del texto del post (longitud del t\'itulo, longitud de la descripci\'on, conteo de palabras).

\subsubsection{Dataset a nivel de Comentario}

Para maximizar la cantidad de filas y cumplir con el requisito de 200K--500K registros, implementamos \texttt{engineer\_comment\_features} que genera un dataset con \textbf{27 columnas} por comentario. Cada fila denormaliza un comentario con su contexto completo: datos del autor del comentario (karma, followers, following, follower ratio), datos del post donde se hizo el comentario (t\'itulo, rating, n\'umero total de comentarios en ese post), datos del autor del post, y la comunidad. Tambi\'en se generan features ingenierizados del texto del comentario como longitud, conteo de palabras y flag de presencia de texto.

\begin{lstlisting}[language=Python, caption={Features a nivel de comentario (simplificado)}]
def engineer_comment_features(comments_lf, posts_lf, users_lf, submolts_lf):
    comment_features = (
        comments_lf
        ...
        .with_columns([
            pl.col("description").str.len_chars().alias("comment_length"),
            pl.col("description").str.split(" ").list.len()
                .alias("comment_word_count"),
        ])
    )
    return comment_features  # 27 columnas por fila
\end{lstlisting}

La funci\'on \texttt{build\_gold\_layer} carga los Parquet de la capa Silver y genera los tres datasets en \texttt{data/gold/}: \texttt{user\_features.parquet} (21 cols), \texttt{post\_features.parquet} (24 cols) y \texttt{comment\_features.parquet} (27 cols).
...
\midrule
\multicolumn{4}{l}{\textbf{Dataset user\_features (21 columnas)}} \\
\midrule
...
\midrule
\multicolumn{4}{l}{\textbf{Dataset comment\_features (27 columnas) -- dataset principal}} \\
\midrule
19 & comment\_length & UInt32 & Longitud del texto del comentario \\
20 & comment\_word\_count & UInt32 & Conteo de palabras del comentario \\
21 & has\_comment\_text & Int32 & Flag binario de presencia de texto \\
22 & comment\_author\_karma & Int64 & Karma del autor del comentario \\
23 & comment\_author\_followers & Int64 & Seguidores del autor del comentario \\
24 & comment\_author\_follower\_ratio & Float64 & Ratio followers/following del autor \\
25 & post\_title & Text & T\'itulo del post donde se coment\'o \\
26 & post\_rating & Int64 & Rating del post \\
27 & post\_total\_comments & UInt32 & Total de comentarios en ese post \\
\bottomrule
\end{tabular}
\caption{Features ingenierizados en la capa Gold (selecci\'on de los m\'as relevantes).}
\label{tab:features}
\end{table}
...
El an\'alisis exploratorio revel\'o una distribuci\'on de karma altamente asim\'etrica: la mayor\'ia de usuarios tienen karma = 0 (mediana = 0), mientras que el m\'aximo es 500,002. La mayor\'ia de los agentes son usuarios placeholder creados a partir de autores de posts y comentarios, con perfiles incompletos que se enriquecen progresivamente mediante la Fase 4 del scraper.
\end{keyinsight}
...
\midrule
scrape & --max-users, --max-posts, --force & Scraping s\'incrono b\'asico \\
scrape-massive & --max-submolts, --workers, --rate-limit & Scraping as\'incrono masivo con workers paralelos \\
enrich-users & --workers, --headless & Enriquecimiento conservador de perfiles incompletos \\
init-db & --postgres & Inicializaci\'on del esquema de la base de datos \\
build & (sin opciones) & Construcci\'on de capas Silver y Gold (3 datasets) \\
...
# Scraping masivo asincrono (3000 submolts, 15 workers)
python -m app scrape-massive --max-submolts 3000 \
    --workers 15 --rate-limit 0.05

# Enriquecer perfiles de usuario incompletos
python -m app enrich-users --workers 3

# Construir capas Silver y Gold (3 datasets)
python -m app build
...
La ejecuci\'on del pipeline de scraping as\'incrono masivo recopil\'o exitosamente datos de moltbook.com utilizando el comando \texttt{scrape-massive} con m\'ultiples workers en paralelo y cross-discovery de submolts:
...
\midrule
Usuarios & $\sim$10,000+ & Perfiles de moltbook.com/u y autores de posts/comments \\
Publicaciones & $\sim$16,000+ & P\'aginas de submolts con scroll \\
Comentarios & $\sim$67,000+ & P\'aginas de posts individuales con scroll \\
Comunidades (SubMolts) & $\sim$3,000+ & moltbook.com/m + cross-discovery \\
User--SubMolt & $\sim$41,000+ & Relaciones descubiertas durante scraping \\
\bottomrule
\end{tabular}
\caption{Estad\'isticas del dataset recopilado (en crecimiento continuo).}
\end{table}
...
La capa Silver limpi\'o y valid\'o los datos crudos, y la capa Gold gener\'o tres datasets de features denormalizados con hasta 27 columnas.

\begin{table}[H]
\centering
\begin{tabular}{@{}llrl@{}}
\toprule
\textbf{Capa} & \textbf{Archivo} & \textbf{Registros} & \textbf{Columnas} \\
\midrule
Silver & users.parquet & $\sim$10,000+ & 8 \\
Silver & posts.parquet & $\sim$16,000+ & 8 \\
Silver & comments.parquet & $\sim$67,000+ & 7 \\
Silver & submolts.parquet & $\sim$3,000+ & 4 \\
\midrule
Gold & user\_features.parquet & $\sim$10,000+ & 21 \\
Gold & post\_features.parquet & $\sim$16,000+ & 24 \\
Gold & comment\_features.parquet & $\sim$67,000+ & 27 \\
\bottomrule
\end{tabular}
\caption{Salidas de las capas de procesamiento.}
\end{table}

El dataset principal para cumplir con el requisito de 200K--500K registros es \texttt{comment\_features.parquet}, que al continuar el scraping overnight con \texttt{scrape-massive --max-submolts 10000} deber\'ia alcanzar las 500K+ filas con 27 columnas cada una.
...
% ============================================================
\clearpage
\section{Arquitectura en AWS}

Para llevar el pipeline a la nube se dise\~n\'o una arquitectura en AWS que replica y escala el flujo de desarrollo local. Se utilizaron estrat\'egicamente los siguientes servicios:

\begin{itemize}
    \item \textbf{S3:} para almacenar el m\'odulo del proyecto empaquetado en un \texttt{.zip}, que luego se importa tanto en el ETL como en el notebook de modelado.
    \item \textbf{RDS con PostgreSQL:} para migrar la base de datos de SQLite (fase de desarrollo) a un motor relacional de producci\'on.
    \item \textbf{AWS Glue Job:} para ejecutar el ETL de scraping. Dado que Playwright necesita acceso a Chrome y Glue no lo soporta, se migr\'o la extracci\'on a las APIs REST de moltbook.
    \item \textbf{CloudWatch:} para monitorear los logs del Glue Job, hacer debugging y optimizar el script. Adem\'as se agreg\'o un reporte de ejecuci\'on al final del proceso.
    \item \textbf{SageMaker:} para la fase de modelado con Jupyter notebooks, reutilizando el m\'odulo de S3.
\end{itemize}

\subsection{S3 -- Almacenamiento del M\'odulo}

Se us\'o S3 para encapsular el proyecto modular en un archivo \texttt{.zip} y subirlo al bucket \texttt{moltbook/library}. Este archivo funciona como librer\'ia compartida: tanto el Glue Job como el notebook de SageMaker lo importan din\'amicamente para reutilizar las clases de \texttt{src.database} y la l\'ogica de entrenamiento sin duplicar c\'odigo (principio DRY).

\subsection{ETL en AWS Glue Job}

\subsubsection{Cambio de Estrategia de Extracci\'on}

Para solucionar el problema de que moltbook.com es una SPA y Playwright necesita acceso a Chrome (no disponible en Glue), se migraron las extracciones a las APIs REST de moltbook:

\begin{itemize}
    \item \texttt{/api/v1/submolts} -- descubrimiento de submolts.
    \item \texttt{/api/v1/posts} -- obtenci\'on de posts por submolt.
    \item \texttt{/api/v1/agents/profile} -- enriquecimiento de perfiles de usuario.
\end{itemize}

\subsubsection{Flujo del ETL}

El proceso se ejecuta en las siguientes fases:

\begin{enumerate}
    \item \textbf{Fase 0 -- Inicializaci\'on:} conexi\'on a PostgreSQL (RDS), creaci\'on idempotente de tablas e inicializaci\'on del fetcher HTTP con rate limiter (1 req/s).

    \item \textbf{Fase 1 -- Descubrimiento de submolts:} fetch de \texttt{/m} para descubrir submolts din\'amicamente, complementado con una lista semilla de 65 submolts conocidos. Se priorizan los nuevos (no existentes en DB).

    \item \textbf{Fase 2 -- Loop por cada submolt:}
    \begin{itemize}
        \item GET \texttt{/api/v1/submolts/\{name\}} para obtener info del submolt y su lista de posts.
        \item Por cada post: se resuelve/crea el usuario autor, se registra la relaci\'on \texttt{user\_submolt}, y si es un post nuevo se inserta en \texttt{posts}.
        \item Por cada post nuevo: GET \texttt{/api/v1/posts/\{id\}} para obtener comentarios y replies recursivas. Se resuelven usuarios y se insertan en \texttt{comments}.
        \item \textbf{Enrichment incremental:} despu\'es de cada submolt, se toman los autores descubiertos y se enriquecen con \texttt{/api/v1/agents/profile} (karma, descripci\'on, followers, following, x\_handle, created\_at). Se hace flush inmediato para resistir interrupciones.
    \end{itemize}

    \item \textbf{Fase 3 -- Sweep final de enrichment:} se consultan todos los usuarios en DB, se filtran los que no fueron enriquecidos en esta ejecuci\'on, y se enriquecen hasta \texttt{MAX\_USERS} restantes.

    \item \textbf{Fase 4 -- Reporte final:} se retorna y loguea el resumen de submolts, usuarios, posts y comentarios nuevos junto con los totales en la base de datos.
\end{enumerate}

\subsubsection{Par\'ametros de Configuraci\'on del ETL}
...
\begin{tabular}{@{}lcl@{}}
\toprule
\textbf{Par\'ametro} & \textbf{Default} & \textbf{Descripci\'on} \\
\midrule
MAX\_SUBMOLTS & 50 & M\'aximo de submolts a procesar \\
MAX\_POSTS & 20 & M\'aximo de posts por submolt \\
MAX\_COMMENTS & 10 & M\'aximo de comentarios por post \\
MAX\_USERS & 100 & M\'aximo de usuarios a enriquecer en sweep final \\
BATCH\_SIZE & 5000 & Tama\~no de chunk para bulk upsert \\
\bottomrule
\end{tabular}
\caption{Par\'ametros de configuraci\'on del ETL en AWS Glue.}
\end{table}

\subsubsection{Beneficios de AWS Glue Job}

\begin{enumerate}
    \item \textbf{Optimizaci\'on de costos:} se us\'o un Job de Python Shell con 1 DPU, que es una fracci\'on del costo de un Job de Spark. La facturaci\'on es serverless: solo se pagan los segundos mientras el script realiza las peticiones a la API y los inserts en RDS.

    \item \textbf{Simplicidad de ingenier\'ia:} se us\'o \texttt{requests} para manipular los JSON de las APIs, junto con la librer\'ia personalizada del proyecto importada desde S3.

    \item \textbf{Automatizaci\'on:} se pueden configurar triggers para que el Job se ejecute peri\'odicamente o cuando se suba un archivo de configuraci\'on a S3, manteniendo el ETL actualizado a lo largo del ciclo de vida de moltbook.

    \item \textbf{Escalabilidad controlada:} de 0.0625 a 1 DPU, lo que permite hasta 16 GB de RAM sin cambiar una sola l\'inea de c\'odigo.
\end{enumerate}

\subsection{Base de Datos -- RDS con PostgreSQL}

Se eligi\'o Amazon RDS con PostgreSQL por los siguientes beneficios:

\begin{itemize}
    \item \textbf{Cero mantenimiento:} AWS se encarga de mantener PostgreSQL actualizado.
    \item \textbf{Escalabilidad:} si el ETL empieza a extraer millones de registros, se puede aumentar almacenamiento o c\'omputo con unos clics.
    \item \textbf{Seguridad integrada:} Security Groups y cifrado en reposo para proteger la informaci\'on.
    \item \textbf{Backups autom\'aticos:} si algo sale mal en el ETL, se puede restaurar la base de datos a cualquier punto en el tiempo.
    \item \textbf{Alta disponibilidad:} si la instancia falla, RDS puede cambiar autom\'aticamente a una r\'eplica en otra zona de disponibilidad.
    \item \textbf{Conectividad flexible:} al ser p\'ublica, se puede acceder desde cualquier lugar (laptop, SageMaker, DBeaver). Idealmente se mover\'ia a un subnet privado con NAT para producci\'on.
    \item \textbf{Extensiones:} PostGIS, \texttt{pg\_stat\_statements}, etc., disponibles con un comando SQL.
\end{itemize}

\begin{warningbox}[Nota sobre la configuraci\'on de red]
Inicialmente se configur\'o el RDS en un subnet privado, pero el Glue Job necesitaba acceso a internet para hacer requests a las APIs de moltbook, y el NAT Gateway no est\'a incluido en el tier gratuito de AWS. Por eso se movi\'o todo al subnet p\'ublico, usando el HOST URL para la conexi\'on en lugar de una conexi\'on privada. En un entorno de producci\'on se agregar\'ia el NAT y se acceder\'ia a la base de datos mediante VPN.
\end{warningbox}

\subsection{SageMaker con Jupyter}

Se centraliz\'o la fase de experimentaci\'on y modelado en AWS SageMaker por las siguientes razones:

\begin{itemize}
    \item \textbf{Experimentaci\'on interactiva:} se utilizaron notebooks de Jupyter para limpiar datos con Polars y, si el volumen escala, se tiene la flexibilidad de activar PySpark sin cambiar de entorno.

    \item \textbf{Modularidad profesional:} se importa din\'amicamente el m\'odulo \texttt{.zip} de S3, reutilizando las clases de \texttt{src.database} y los modelos de entrenamiento sin duplicar c\'odigo.

    \item \textbf{Conectividad de baja latencia:} al estar dentro de la red de AWS, la comunicaci\'on con PostgreSQL en RDS es inmediata, sin cuellos de botella externos.

    \item \textbf{Escalabilidad de c\'omputo:} se puede programar en una instancia econ\'omica y, al momento de entrenar modelos pesados, escalar a instancias con m\'as RAM o GPU con un clic.

    \item \textbf{Persistencia y MLOps:} SageMaker mantiene el workspace guardado. Una vez que el modelo sea preciso, se puede desplegarlo como un endpoint de producci\'on para que una aplicaci\'on lo consuma.
\end{itemize}

Se evalu\'o tambi\'en usar Glue Job para el modelado, pero SageMaker result\'o m\'as econ\'omico y flexible para esta fase de experimentaci\'on.
...
\begin{enumerate}
    \item Se dise\~n\'o un \textbf{scraper robusto y escalable con Playwright} tanto s\'incrono como as\'incrono con m\'ultiples workers en paralelo, capaz de extraer datos de una SPA renderizada con JavaScript, con rate limiting, cach\'e HTML, reintentos con backoff exponencial y cross-discovery de submolts.

    \item Se implement\'o una \textbf{base de datos relacional PostgreSQL en AWS RDS} con esquema normalizado de 5 tablas, foreign keys, 6 \'indices de rendimiento y operaciones CRUD gen\'ericas con upsert masivo utilizando \texttt{psycopg2}.

    \item Se aplic\'o la \textbf{arquitectura Medallion con Polars Lazy} evaluation, generando tres datasets denormalizados (usuario, post y comentario) con hasta 27 columnas para un total potencial de 500K+ registros.
    ...
    \item Se dise\~n\'o una \textbf{arquitectura en AWS} con S3, Glue Job, RDS y SageMaker que permite llevar el pipeline completo a la nube, migrando la extracci\'on a APIs REST y centralizando el modelado en notebooks con acceso directo a la base de datos.
\end{enumerate}
...
PostgreSQL (AWS RDS) & SQLite & Concurrencia de workers, escala en la nube \\
...
AWS Glue (Python Shell) & Glue Spark / Lambda & Costo m\'inimo, librer\'ias est\'andar \\
SageMaker & Glue Job para modelado & Flexibilidad, notebooks interactivos \\
\bottomrule
\end{tabular}
...
\midrule
AWS Glue & Servicio serverless de AWS para ETL que ejecuta scripts de Python o Spark sin necesidad de aprovisionar servidores. \\
AWS RDS & Relational Database Service: servicio administrado de bases de datos relacionales en AWS. \\
AWS S3 & Simple Storage Service: almacenamiento de objetos en la nube de AWS. \\
AWS SageMaker & Plataforma de AWS para construir, entrenar y desplegar modelos de machine learning con notebooks Jupyter integrados. \\
...
CloudWatch & Servicio de monitoreo de AWS para logs, m\'etricas y alarmas de recursos en la nube. \\
DPU & Data Processing Unit: unidad de c\'omputo de AWS Glue que provee 4 vCPUs y 16 GB de memoria. \\
...
WAL & Write-Ahead Logging: modo de journal que mejora el rendimiento de escritura concurrente en bases de datos. \\
\bottomrule
\end{tabular}
...
python -m app init-db --postgres

# 3. Scraping masivo asincrono (3000+ submolts)
python -m app scrape-massive --max-submolts 3000 \
    --workers 15 --rate-limit 0.05

# 4. (Opcional) Enriquecer perfiles incompletos
python -m app enrich-users --workers 3

# 5. Verificar estado del scraping
python -m app status

# 6. Construir capas Silver y Gold (3 datasets)
python -m app build -v

# 7. Entrenar modelo con H2O AutoML
